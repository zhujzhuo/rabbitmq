...
-type(amqp_method_field_name() ::
       ( frame_max | arguments | message_count | reason
       | global | mandatory | mechanisms | no_ack
       | consumer_tag | queue | no_local | realm
       | drain | delivery_tag | active | arguments
       | routing_key | class_id | capabilities | locales
       | if_unused | queue | queue | heartbeat
       | type | channel_max | routing_key | delivery_tag
       | exchange | arguments | queue | durable
       | heartbeat | out_of_band | message_count | method_id
       | queue | insist | exclusive | consumer_tag
       | nowait | no_ack | read | challenge
       | requeue | arguments | nowait | ticket
       | message_count | exchange | if_empty | reply_code
       | exchange | ticket | exchange | exchange
       | queue | exchange | consumer_tag | ticket
       | destination | redelivered | channel_max | passive
       | consumer_tag | reply_text | durable | write
       | active | nowait | immediate | exclusive
       | nowait | passive | consumer_count | nowait
       | delivery_tag | ticket | auto_delete | ticket
       | reply_text | cluster_id | channel_id | destination
       | ticket | source | client_properties | frame_max
       | delivery_tag | requeue | queue | routing_key
       | ticket | routing_key | nowait | arguments
       | response | version_major | ticket | credit_drained
       | requeue | class_id | prefetch_size | mechanism
       | source | requeue | ticket | routing_key
       | method_id | exchange | if_unused | internal
       | redelivered | available | known_hosts | response
       | passive | delivery_tag | multiple | version_minor
       | consumer_tag | arguments | active | ticket
       | routing_key | arguments | prefetch_count | routing_key
       | reply_code | ticket | nowait | nowait
       | nowait | auto_delete | locale | server_properties
       | exclusive | ticket | ticket | consumer_tag
       | message_count | reply_code | queue | consumer_tag
       | nowait | exchange | nowait | credit
       | reply_text | routing_key | multiple | virtual_host )).
