...
-type(amqp_method_field_name() ::
       ( reply_code | consumer_tag | result | multiple
       | consumer_tag | redelivered | type | nowait
       | no_local | prefetch_count | mandatory | consumer_tag
       | known_hosts | durable | dtx_identifier | exchange
       | exclusive | delivery_tag | frame_max | arguments
       | cluster_id | locale | content_checksum | no_local
       | mandatory | server_properties | read | consumer_tag
       | reply_text | consumer_tag | active | nowait
       | no_ack | reply_text | exchange | message_count
       | exchange | queue | message_count | queue
       | routing_key | result | passive | routing_key
       | routing_key | if_unused | routing_key | requeue
       | internal | reply_code | exchange | consumer_tag
       | routing_key | delivery_tag | mechanisms | consume_rate
       | immediate | class_id | write | exchange
       | nowait | redelivered | arguments | operation
       | message_count | global | reply_code | challenge
       | nowait | table | insist | reply_text
       | delivery_tag | nowait | queue | ticket
       | exchange | ticket | consumer_tag | active
       | heartbeat | routing_key | consumer_tag | response
       | ticket | ticket | string_1 | reply_code
       | locales | exchange | ticket | ticket
       | consumer_tag | method_id | consumer_count | reply_text
       | consumer_tag | durable | integer_3 | reply_code
       | exclusive | string_op | global | message_count
       | prefetch_count | exclusive | delivery_tag | channel_max
       | integer_2 | reply_code | exchange | host
       | delivery_tag | string_2 | ticket | requeue
       | queue | queue | meta_data | nowait
       | staged_size | multiple | passive | routing_key
       | exchange | no_local | ticket | routing_key
       | integer_4 | arguments | nowait | class_id
       | routing_key | mandatory | integer_1 | known_hosts
       | queue | frame_max | nowait | delivery_tag
       | response | queue | exchange | exchange
       | queue | auto_delete | identifier | identifier
       | exchange | auto_delete | nowait | consumer_tag
       | details | channel_max | nowait | exchange
       | operation | reply_text | active | exclusive
       | virtual_host | method_id | delivery_tag | prefetch_count
       | exclusive | immediate | nowait | heartbeat
       | delivery_tag | string_result | passive | if_empty
       | consumer_tag | requeue | prefetch_size | mechanism
       | queue | version_major | prefetch_size | global
       | immediate | identifier | no_ack | redelivered
       | client_properties | no_ack | routing_key | integer_op
       | ticket | requeue | prefetch_size | ticket
       | exchange | consumer_tag | routing_key | arguments
       | version_minor | content_size | ticket | integer_result
       | ticket | queue | nowait | if_unused
       | consumer_tag | queue | ticket | ticket
       | ticket | out_of_band | consumer_tag | realm
       | capabilities | reply_text )).
